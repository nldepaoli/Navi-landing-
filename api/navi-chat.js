...
The opening line — either the standard "What brings you here — a tree, a place, a project, something that happened, or just curious what this is?" or, for a recognized returning visitor, a brief "welcome back" line — is already handled for you and will appear as your own first turn in the conversation history you're given. Never say it again, reintroduce yourself, or restate it in any form after that, whichever version it was — respond to whatever the person actually said next, every time, no exceptions.
...
async function updateTranscript(sessionId, messages, isFirstTurn) {
  const base = process.env.AIRTABLE_BASE_ID;
  const table = encodeURIComponent(process.env.AIRTABLE_TABLE_NAME || "Conversations");
  const headers = {
    Authorization: `Bearer ${process.env.AIRTABLE_TOKEN}`,
  ...
  const currentVisitTranscript = formatTranscript(messages);
  if (!currentVisitTranscript) return;
  ...
  const payloadFields = { full_transcript: currentVisitTranscript, session_id: sessionId, last_updated: new Date().toISOString() };

  // Only at the true start of a new visit — not on every exchange within
  // one — check whether a prior visit's transcript is sitting in
  // full_transcript and archive it first. This is what actually lets
  // Airtable hold a real, growing history across visits instead of each
  // new one silently destroying the last, while still keeping
  // full_transcript itself simple (just "this visit," overwritten
  // normally turn to turn, exactly as before).
  if (isFirstTurn && existingFields.full_transcript) {
    const stamp = new Date().toISOString();
    const archived = existingFields.past_visits_transcript
      ? `${existingFields.past_visits_transcript}\n\n--- visit ended ${stamp} ---\n\n${existingFields.full_transcript}`
      : `--- visit ended ${stamp} ---\n\n${existingFields.full_transcript}`;
    payloadFields.past_visits_transcript = archived;
  }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (existingContact) {
      ...
      if (existingContact.full_transcript) {
        const t = existingContact.full_transcript;
        const recent = t.length > 3000 ? `...(earlier part of that conversation omitted)...\n${t.slice(-3000)}` : t;
        bits.push(`What you actually talked about last time (their most recent visit):\n${recent}`);
      }
      if (existingContact.past_visits_transcript) {
        const p = existingContact.past_visits_transcript;
        const recentPast = p.length > 3000 ? `...(older visits omitted)...\n${p.slice(-3000)}` : p;
        bits.push(`From earlier visits before that, for additional context if it's relevant:\n${recentPast}`);
      }
      systemPrompt += `\n\nThis is someone you've talked with before, on this same device — you already know this about them:\n${bits.join("\n")}\nIf they ask what you talked about before, or where they left off, actually answer from what's here — you genuinely do know, so don't say you don't or that this is a gap. If it fits naturally otherwise, let a real sense of remembering them come through — not a scripted "welcome back" line, just genuine familiarity. Don't re-ask things you already know here. But don't force the acknowledgment either if the conversation's actual opening doesn't call for it.`;
    }
    ...
    try {
      await updateTranscript(sessionId, convo, isFirstTurn);
    } catch (e) {
