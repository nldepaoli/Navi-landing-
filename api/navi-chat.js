...
Never use markdown formatting — no **bold**, no _italic_, no markdown-style links. This surface only ever displays plain text, so any markdown syntax shows up literally as stray asterisks or underscores rather than rendering. If you want to emphasize a link or email, just say it plainly.
...
// Fires an email alert via Resend. Silently does nothing if the env vars
// aren't set, so this never becomes its own point of failure. Deliberately
// not awaited by callers in the hot path where it'd add latency to an
// already-failing request isn't a concern — these are rare, so a small
// delay here is fine.
...
module.exports = async function handler(req, res) {
  ...
  try {
      ...
      for (const t of toolUses) {
        if (t.name === "log_contact") {
          try { await upsertContact(sessionId, t.input); }
            ...
            notifyError("Contact failed to save — data below can be added by hand", `A visitor's info didn't save to Airtable (the conversation itself was fine for them).\n\nWHAT WAS BEING SAVED (copy this into Airtable manually if you want to recover it):\n${JSON.stringify(t.input, null, 2)}\n\nSession: ${sessionId}\n\nError: ${e && e.stack ? e.stack : e}`);
          }
        } else if (t.name === "save_knowledge") {
          try { await saveKnowledge(t.input); }
          catch (e) {
            console.error("Knowledge save failed:", e);
            notifyError("Knowledge entry failed to save — data below can be added by hand", `A save_knowledge call didn't save to Airtable.\n\nWHAT WAS BEING SAVED (copy this into Airtable manually if you want to recover it):\n${JSON.stringify(t.input, null, 2)}\n\nSession: ${sessionId}\n\nError: ${e && e.stack ? e.stack : e}`);
          }
    ...
    notifyError("Chat request failed — a visitor got the fallback message", `Someone's message failed to get a real reply and they saw the generic "went a little quiet" fallback instead.\n\nNothing to manually recover here — if this keeps happening, paste this whole email into a conversation with Claude to get it debugged and fixed.\n\nSession: ${sessionId}\n\nError: ${err && err.stack ? err.stack : err}`);
